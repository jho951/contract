...
- EC2 배포 명령: CI/CD는 항상 `/opt/deploy`에서 `./scripts/deploy-stack.sh up <service>`만 호출하고, 초기 수동 기동이 필요할 때만 전체 `./scripts/deploy-stack.sh up`를 사용한다.
...
- 실제 bootstrap 템플릿은 [../../infra/templates/single-ec2/user_data.sh.tftpl](../../infra/templates/single-ec2/user_data.sh.tftpl)을 기준으로 한다.
- 7개 서비스 자동 배포까지 포함한 완성형은 [../../infra/templates/single-ec2/user_data.full-stack.sh.tftpl](../../infra/templates/single-ec2/user_data.full-stack.sh.tftpl)을 기준으로 한다.
- 앱 레포 clone 없이 바로 올리려면 [../../infra/templates/single-ec2/deploy-bundle/README.md](../../infra/templates/single-ec2/deploy-bundle/README.md)의 deploy bundle을 우선 사용한다.
- deploy bundle 기반 CD 명령은 `cd /opt/deploy && ./scripts/deploy-stack.sh up gateway-service`처럼 서비스 단건 반영으로 고정한다.
...
- compose override 템플릿은 [../../infra/templates/single-ec2/README.md](../../infra/templates/single-ec2/README.md)를 기준으로 복사한다.
- 실제 서비스 레포 실행은 [../../infra/scripts/single-ec2/deploy-single-ec2-service.sh](../../infra/scripts/single-ec2/deploy-single-ec2-service.sh) 또는 [../../infra/scripts/single-ec2/deploy-single-ec2-stack.sh](../../infra/scripts/single-ec2/deploy-single-ec2-stack.sh)를 기준으로 한다.
...
서비스별 `.env.prod` 예시 파일은 [../../infra/templates/single-ec2/env/](../../infra/templates/single-ec2/env/)를 기준으로 사용한다.
