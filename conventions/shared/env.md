...
- Gateway 전용 추가 환경변수와 서비스별 세부 env 문서는 각 구현 레포가 소유하고, 이 문서는 cross-service baseline만 다룬다.
...
### 현재 구현 기준 운영 권장
- `INTERNAL_API_KEY`
- `SPRING_SECURITY_OAUTH2_CLIENT_REGISTRATION_GITHUB_REDIRECT_URI`
- `SSO_EXPLAIN_ORIGIN`
...
- 운영 callback URI는 `http`가 아니라 `https` 기준으로 맞춘다.
...
### 현재 구현 기준 운영 권장
- `GATEWAY_LOGIN_RATE_LIMIT_PER_MINUTE`
- `GATEWAY_MAX_BODY_BYTES`
- `AUTH_JWT_VERIFY_ENABLED=true`
- `AUTH_JWT_SHARED_SECRET=<auth-service AUTH_JWT_SECRET와 동일한 값 또는 동등한 검증 키>`
- `GATEWAY_FORWARD_AUTHORIZATION_HEADER=false`
- `PLATFORM_SECURITY_AUTH_JWT_SECRET`
- `PLATFORM_SECURITY_IP_GUARD_ENABLED=true`
- `PLATFORM_SECURITY_IP_GUARD_ADMIN_RULES_0`
- `PLATFORM_SECURITY_IP_GUARD_INTERNAL_RULES_0`
...
### 현재 구현 기준 운영 권장
- `AUTHZ_PLATFORM_GOVERNANCE_AUDIT_SERVICE_NAME=authz-service`
- `AUTHZ_PLATFORM_GOVERNANCE_AUDIT_ENVIRONMENT=<environment>`
- `AUTHZ_PLATFORM_SECURITY_SERVICE_ROLE_PRESET=INTERNAL_SERVICE`
- `AUTHZ_PLATFORM_SECURITY_RATE_LIMIT_ENABLED=false`

### 호환성(Compatibility)
- 일부 구현/terraform 경로에는 `PERMISSION_MYSQL_*`, `PERMISSION_SPRING_DATASOURCE_*` 같은 legacy prefix가 남아 있을 수 있다.
- 현재 CI와 single-EC2 예제는 legacy prefix도 같은 값으로 함께 둔다.
...
- `AUTHZ_PLATFORM_SECURITY_SERVICE_ROLE_PRESET`, `AUTHZ_PLATFORM_GOVERNANCE_AUDIT_SERVICE_NAME` 같은 platform env는 공용 `.env.backend`에만 두지 말고 compose에서 Authz 전용 값으로 다시 고정한다.
