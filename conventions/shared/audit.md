...
- 서비스별 상세 이벤트 목록은 각 구현 레포 문서에서 관리한다.
- 공통 모듈 구현 세부는 [Module Ecosystem](../../services/registry/module-ecosystem.md)의 `audit-log` 항목을 따른다.
- 저장소, 보존 기간, 조회 API가 별도 서비스로 분리되면 이 문서를 기준으로 서비스별 계약을 추가한다.

## Contract Placement
- 공통 감사 원칙: `conventions/shared/audit.md`
- 서비스별 감사 이벤트: 구현 레포 문서
- 감사 모듈/라이브러리 의존성: `services/registry/module-ecosystem.md`
