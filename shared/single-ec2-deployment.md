...
- 현재는 단일 EC2 리소스 안에서 7개 서비스를 함께 안정적으로 운영해야 한다.
...
- EC2 배포 명령: `docker compose pull && docker compose up -d`
...
- inbound `8080/tcp`: reverse proxy 없이 gateway를 직접 공개할 때만 제한적으로 허용
- inbound `3000/tcp`: Grafana를 외부에서 직접 볼 때만 운영자 IP로 제한
...
| --- | --- | --- | --- | --- |
| `gateway-service` | `8080` | `80:8080` 또는 `8080:8080` | 예 | 가능하면 reverse proxy 뒤에 둔다 |
...
| `grafana` | `3000` | 선택 | 제한적 | 운영자 IP 제한 권장 |
...
| Redis | `redis` 또는 `central-redis` 중 compose 기준 canonical alias |

권장 canonical alias:

- Redis compose service key는 `redis`
- 필요한 경우 legacy alias `central-redis`를 추가 network alias로만 유지
