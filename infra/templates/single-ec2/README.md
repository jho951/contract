...
| [../../scripts/single-ec2/](../../scripts/single-ec2/) | 실제 서비스 레포를 순서대로 올리는 단일 EC2 실행 스크립트의 canonical source |
...
4. 자동 모드에서 실제 렌더링은 `scripts/single-ec2/render-full-stack-user-data.sh`를 사용한다.
5. 수동 모드에서는 각 서비스 repo `.env.prod`를 준비하고 ECR image URI를 채운다.
6. `scripts/single-ec2/deploy-single-ec2-service.sh` 또는 `scripts/single-ec2/deploy-single-ec2-stack.sh`로 실행한다.
7. `service-backbone-shared` network에 모든 서비스를 붙인다.
8. 전체 초기 기동은 `redis -> auth -> user -> authz -> editor -> gateway` 순으로 이미지를 pull한 뒤 실행한다.
9. 운영 중 단일 서비스 반영은 `scripts/single-ec2/deploy-single-ec2-service.sh <service-name> <repo-dir> up`처럼 대상 서비스만 갱신한다.
10. 외부 공개는 `nginx.single-ec2.conf.example` 또는 [../../../conventions/shared/single-ec2-edge-routing.md](../../../conventions/shared/single-ec2-edge-routing.md) 기준으로 Nginx에 붙인다.
...
```bash
./scripts/single-ec2/deploy-single-ec2-service.sh \
  auth-service \
...
./scripts/single-ec2/deploy-single-ec2-stack.sh up
```
...
5. 서비스별 `pull/up`를 순서대로 수행하는 `scripts/single-ec2/deploy-single-ec2-stack.sh up` 실행
...
```bash
./scripts/single-ec2/render-full-stack-user-data.sh \
