...
- runtime `scripts/`: `/opt/deploy/scripts`에 배치되는 실행 스크립트. canonical source는 [../../../scripts/deploy-bundle/](../../../scripts/deploy-bundle/)가 소유한다.
...
- service repo workflow는 `contract.lock.yml`로 fetch한 `.contract/templates/single-ec2/deploy-bundle/` asset과 `.contract/scripts/deploy-bundle/` runtime script를 함께 `/opt/deploy`에 동기화한 뒤 같은 경로의 스크립트를 호출한다.
...
/tmp/service-contract/scripts/deploy-bundle/bootstrap-ec2.sh /opt/deploy
...
- `editor-mysql`은 빈 volume으로 처음 올라올 때 `documents`, `blocks`, `document_resources`, `platform_resource_catalog`, `platform_resource_outbox` baseline schema를 자동 초기화한다. 기존 volume을 재사용하는 경우 init SQL은 다시 실행되지 않으므로, 스키마 누락이면 `config/editor-mysql/editor-schema.sql`을 수동 적용해야 한다.
