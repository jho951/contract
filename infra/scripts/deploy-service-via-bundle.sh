...
DEPLOY_BUNDLE_TEMPLATE_ROOT="$CONTRACT_ROOT/templates/single-ec2/deploy-bundle"
DEPLOY_BUNDLE_SCRIPT_ROOT="$CONTRACT_ROOT/scripts/deploy-bundle"
BUNDLE_STAGING_DIR=""

cleanup() {
  if [[ -n "$BUNDLE_STAGING_DIR" && -d "$BUNDLE_STAGING_DIR" ]]; then
    rm -rf "$BUNDLE_STAGING_DIR"
  fi
}
trap cleanup EXIT

prepare_bundle_staging_dir() {
  BUNDLE_STAGING_DIR="$(mktemp -d)"
  cp -R "$DEPLOY_BUNDLE_TEMPLATE_ROOT/." "$BUNDLE_STAGING_DIR/"
  mkdir -p "$BUNDLE_STAGING_DIR/scripts"
  cp "$DEPLOY_BUNDLE_SCRIPT_ROOT/"*.sh "$BUNDLE_STAGING_DIR/scripts/"
  chmod +x "$BUNDLE_STAGING_DIR/scripts/"*.sh
}
...
[[ -d "$DEPLOY_BUNDLE_TEMPLATE_ROOT" ]] || {
  echo "Deploy bundle template not found: $DEPLOY_BUNDLE_TEMPLATE_ROOT" >&2
  exit 1
}
[[ -d "$DEPLOY_BUNDLE_SCRIPT_ROOT" ]] || {
  echo "Deploy bundle scripts not found: $DEPLOY_BUNDLE_SCRIPT_ROOT" >&2
  exit 1
}
...
prepare_bundle_staging_dir
tar -C "$BUNDLE_STAGING_DIR" -cf - . | ssh "${ssh_args[@]}" "$DEPLOY_USER@$DEPLOY_HOST" \
