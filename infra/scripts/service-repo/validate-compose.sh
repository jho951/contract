...
usage() {
  echo "Usage: validate-compose.sh <dev|prod>" >&2
  exit 1
}

TARGET_ENV="${1:-}"
[[ -n "$TARGET_ENV" ]] || usage
[[ "$TARGET_ENV" == "dev" || "$TARGET_ENV" == "prod" ]] || usage

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
if [[ ! -f "$REPO_ROOT/contract.lock.yml" ]]; then
  REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
fi
...
"$CONTRACT_DIR/scripts/validate-service-compose.sh" \
