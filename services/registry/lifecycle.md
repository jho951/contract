...
## 변경 순서
### 1) 요구사항 정의
- 공통 가이드라인 진입점으로 `conventions/shared/common-guidelines.md`를 먼저 확인한다.
- 브라우저/비브라우저
- 라우트/헤더
- 인증/보안
- 캐시
- 응답/에러
- 환경변수/OpenAPI
- 권한/기능/정책 판단은 `conventions/shared/decision-criteria.md`를 먼저 본다.
...
- 인증 채널 정책 변경
  ...
  - `services/artifacts/openapi/v1/user-service.yaml`
  - 구현 레포의 auth/user security 문서
...
## 공통 판단 기준
- 새 권한이나 기능을 추가할 때는 서비스별 문서보다 먼저 `conventions/shared/decision-criteria.md`를 확인한다.
