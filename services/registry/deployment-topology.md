...
EDITOR_SERVICE_URL: http://10.0.x.x:8083
...
## 현재 서비스 코드 기준 확인
2026-04-23 로컬 서비스 레포 기준 확인 결과다.
...
| `authz-service` | base/prod compose service key `authz-service:8084`, canonical DB host는 `authz-mysql`, Redis는 external `redis` 사용 | `REDIS_HOST`는 EC2-3 private IP/DNS로 설정한다. prod bundle은 `AUTHZ_SPRING_DATASOURCE_*`로 `authz-mysql` 연결을 명시하고, service repo prod compose에는 동일 datasource env 보강이 필요하다. env/terraform에는 `PERMISSION_*` 계열 legacy env prefix가 아직 남아 있을 수 있다. |
...
## 서비스 레포 반영 사항
2026-04-23 기준으로 EC2 3대 배포를 위해 서비스 레포의 prod 설정은 다음 방향으로 정렬한다.
...
| `authz-service` | prod compose의 embedded Redis를 제거하고 `REDIS_HOST`를 required로 둔다. single-EC2 bundle은 `authz-mysql`을 함께 두고 `AUTHZ_SPRING_DATASOURCE_*`를 명시한다. prod service key는 `authz-service`이고 publish 변수는 `AUTHZ_SERVICE_HOST_BIND`, `AUTHZ_SERVICE_HOST_PORT`를 사용한다. |
